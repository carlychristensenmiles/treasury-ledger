...
```
app/
  main.py            FastAPI app setup, session middleware, routing, auto-seed-on-first-boot
  ...
  seed.py              shared fund-CSV-loading logic (used by scripts/seed_db.py and app startup)
...
scripts/seed_db.py    CLI wrapper around app/seed.py, for local/manual re-seeding
tests/                 pytest suite
Dockerfile, fly.toml, DEPLOY.md   Fly.io deployment (see "Deploying" below)
```

## Deploying

`Dockerfile`, `fly.toml`, and `DEPLOY.md` set this up for Fly.io specifically
— chosen because the app needs a real system package (Tesseract OCR, not
just a `pip install`) and a persistent disk for the SQLite database, which
rules out fully serverless platforms like Vercel's standard Python runtime
(read-only filesystem, no apt step; see `DEPLOY.md` for the specifics). The
app seeds its own fund database automatically on first boot against an empty
database, so a fresh deploy doesn't need a manual data-loading step.
